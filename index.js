...
layui
	.extend({
		setter: "lib/config", //配置文件
		admin: "lib/admin", //核心模块
		view: "lib/view", //核心模块
		apiPath: "lib/apiPath", //核心模块
	})
	.define(["setter", "admin", "table", "apiPath"], function (exports) {
			...
			renderPage = function () {
				...
				table.set({
					...
					parseData: function (res) {
						//res 即为原始返回的数据
						admin.events.checkLogin(res);
						let params = {};
						if (res.data.data) {
							params = {
								code: 0,
								count: res.data.total ? res.data.total : res.data.data.length, //解析数据长度
								data: res.data.data ? res.data.data : [], //解析数据列表
				...
				if (isIndPage || pathURL === "/login") {
					...
					if (setter.interceptor) {
						...
						if (!local[setter.request.tokenName]) {
							return (location.hash = "/login"); //跳转到登入页  /redirect=' + encodeURIComponent(pathURL)
						}
					}
		...
		//初始主体结构
		layui.link(
			setter.base + "../res/adminui/dist/css/admin.css?v=" + (admin.v + "-1"),
			function () {
				entryPage();
			},
			"layuiAdmin",
