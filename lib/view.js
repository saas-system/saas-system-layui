...
layui.define(['laytpl', 'layer'], function(exports) {
    ...
    view.req = function(options) {
        let loading,
            ...
            error = options.error,
            is_loading = options.loading !== undefined ? options.loading : true,
        ...
        return $.ajax($.extend({
            ...
            success: function(res) {
                ...
                //登录状态失效，清除本地 Authorization，并强制跳转到登入页
                ...
                //其它异常
                    ...
                    let errorText = [
                        (res[response.msgName] || '返回状态码异常'), debug()
                    ].join('');
    ...
    view.popup = function(options) {
        ...
        return layer.open($.extend({
            ...
            success: function(layero, index) {
                $('.layui-layer-admin i[close]').remove();
                $('.layui-layer-setwin a').css({ background: 'none', color: '#FFF', fontSize: '15px', lineHeight: '22px' }).addClass('layui-icon').addClass('layui-icon-close');
