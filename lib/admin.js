...
layui.define('view', function (exports) {
    let $ = layui.jquery,
        ...
        APP_BODY = '#LAY_app_body', APP_FLEXIBLE = 'LAY_app_flexible', FILTER_TAB_TBAS = 'layadmin-layout-tabs',
        APP_SPREAD_SM = 'layadmin-side-spread-sm', TABS_BODY = 'layadmin-tabsbody-item', ICON_SHRINK = 'layui-icon-shrink-right',
        ICON_SPREAD = 'layui-icon-spread-left', SIDE_SHRINK = 'layadmin-side-shrink', SIDE_MENU = 'LAY-system-side-menu'
        ...
        , admin = {
            ...
            // 获取数据表格已选列数据集
            ,
            getCheckedId: function (filter, field = 'id') {
                let checkData = layui.table.checkStatus(filter).data; //得到选中的数据
                let arr = [];
                let arrStr = '';
            ...
            //获取列表
            ,
            getList: function (elem, path, where, title, val, dv) {
                if (!elem) {
                    return;
                }
                val = val ? val : 'id';
                    ...
                    success(res) {
                        if (res.data.length > 0) {
                            $.each(res.data, function (index, item) {
                                $(elem).append(`<option postCode="${item['post_code']}" value="${item[val]}">${item[title]}</option>`);
                            });
                            if (dv) {
                                $(elem).val(dv);
                            }
                        } else {
                            $(elem).html(`<option value="">无数据</option>`);
                        }
            ...
            // 业务归属下拉
            ,
            business: function (filters = {department: '', group: '', account: '', shop: ''}, req) {
                let self = this;
                req = req ? '请选择' : '全部';

                let is_super_admin = layui.data(layui.setter.tableName)['is_super_admin'];
                let is_self = layui.data(layui.setter.tableName)['is_self'];
                let department_id = layui.data(layui.setter.tableName)['department_id'];
                let group_id = layui.data(layui.setter.tableName)['group_id'];
                let user_id = layui.data(layui.setter.tableName)['user_id'];

                if (filters.department) {
                    $('#' + filters.department).html(`<option class="default" value="">${req}业务部</option>`);
                }
                if (filters.group) {
                    $('#' + filters.group).html(`<option class="default" value="">${req}业务组</option>`);
                }
                if (filters.account) {
                    $('#' + filters.account).html(`<option class="default" value="">${req}账户</option>`);
                }
                if (filters.shop) {
                    $('#' + filters.shop).html(`<option class="default" value="">${req}店铺</option>`);
                }

                if (is_super_admin) {
                    //获取业务部列表
                    self.getList('#' + filters.department, layui.apiPath.userBusinessDepartmentList);
                } else {
                    if (filters.department) {
                        //获取业务部列表
                        self.getList('#' + filters.department, layui.apiPath.userBusinessDepartmentList);
                        if (department_id > 0) {
                            $('#' + filters.department + '>.default').remove();
                            $('#' + filters.department).val(department_id);
                        }
                    }
                    ...
                    if (department_id > 0) {
                        if (filters.group) {
                            // 获取业务组列表
                            self.getList('#' + filters.group, layui.apiPath.userBusinessGroupList, {department_id: department_id});
                            if (group_id > 0) {
                                $('#' + filters.group + '>.default').remove();
                                $('#' + filters.group).val(group_id);
                            }
                        }
                    }
                    if (group_id > 0) {
                        if (filters.account) {
                            // 获取账户列表
                            self.getList('#' + filters.account, layui.apiPath.userAccountList, {group_id: group_id}, 'username');
                            if (is_self > 0) {
                                $('#' + filters.account + '>.default').remove();
                                $('#' + filters.account).val(user_id);
                            }
                        }
                    }
                    if (is_self > 0 && filters.shop) {
                        // 获取店铺列表
                        self.getList('#' + filters.shop, layui.apiPath.userShopList, {user_id: user_id}, 'shop_name');
                    }
                }
                ...
                if (is_self > 0) {
                    $('#' + filters.department).attr('readonly', 'readonly');
                    $('#' + filters.group).attr('readonly', 'readonly');
                    $('#' + filters.account).attr('readonly', 'readonly');
                } else {
                    if (filters.department) {
                        //监听业务部下拉
                        layui.form.on('select(' + filters.department + ')', function (data) {
                            if (filters.group) {
                                if (is_super_admin || group_id < 1) {
                                    $('#' + filters.group).html(`<option value="">${req}业务组</option>`);
                                } else {
                                    $('#' + filters.group).empty();
                                }
                                if (data.value) {
                                    // 获取业务组列表
                                    self.getList('#' + filters.group, layui.apiPath.userBusinessGroupList, {department_id: data.value});
                                    if (group_id > 0) {
                                        $('#' + filters.group).val(group_id);
                                    }
                                }
                            ...
                            if (filters.account) {
                                if (is_super_admin || is_self < 1) {
                                    $('#' + filters.account).html(`<option value="">${req}账户</option>`);
                                } else {
                                    $('#' + filters.account).empty();
                                }
                            }
                            if (filters.shop) {
                                $('#' + filters.shop).html(`<option value="">${req}店铺</option>`);
                            }
                            layui.form.render();
                        });
                    }
                    ...
                    if (filters.group) {
                        //监听业务组下拉
                        layui.form.on('select(' + filters.group + ')', function (data) {
                            if (filters.account) {
                                if (is_super_admin || is_self < 1) {
                                    $('#' + filters.account).html(`<option value="">${req}账户</option>`);
                                } else {
                                ...
                                if (data.value) {
                                    // 获取账户列表
                                    self.getList('#' + filters.account, layui.apiPath.userAccountList, {group_id: data.value}, 'username');
                                    if (is_self > 0) {
                                        $('#' + filters.account).val(user_id);
                                    }
                                }
                    ...
                    if (filters.account) {
                        //监听账户下拉
                        layui.form.on('select(' + filters.account + ')', function (data) {
                            if (filters.shop) {
                                $('#' + filters.shop).html(`<option value="">${req}店铺</option>`);
                                if (data.value) {
                                    // 获取店铺列表
                                    self.getList('#' + filters.shop, layui.apiPath.userShopList, {user_id: data.value}, 'shop_name');
                                }
                            }
                            layui.form.render();
                        });
            ...
            // 业务归属下拉
            // where = {department_id,group_id,user_id,shop_id}
            ,
            businessFrom: function (filters = {department: '', group: '', account: '', shop: ''}, where) {
                let self = this;
                let req = '请选择';
                let is_self = layui.data(layui.setter.tableName)['is_self'];
                let department_id = layui.data(layui.setter.tableName)['department_id'];
                let group_id = layui.data(layui.setter.tableName)['group_id'];
                let user_id = layui.data(layui.setter.tableName)['user_id'];
                let select_department_id, select_group_id, select_user_id, select_shop_id;
                if (where) {
                    department_id = select_department_id = where.department_id ? where.department_id : '';
                    group_id = select_group_id = where.group_id ? where.group_id : '';
                    user_id = select_user_id = where.user_id ? where.user_id : '';
                    select_shop_id = where.shop_id ? where.shop_id : '';
                } else {
                    if (is_self > 0) {
                        select_department_id = department_id;
                ...
                if (filters.department) {
                    if (is_self === 1) {
                        $('#' + filters.department).empty();
                    } else {
                        $('#' + filters.department).html(`<option class="default" value="">${req}业务部</option>`);
                    }
                    //获取业务部列表
                    self.getList('#' + filters.department, layui.apiPath.userBusinessDepartmentList, {}, '', '', select_department_id);
                }
                if (filters.group) {
                    if (is_self === 1) {
                        $('#' + filters.group).empty();
                    ...
                    // 获取业务组列表
                    if ((is_self > 0 && department_id) || select_department_id) {
                        self.getList('#' + filters.group, layui.apiPath.userBusinessGroupList, {department_id: department_id}, '', '', select_group_id);
                    }
                }
                if (filters.account) {
                    if (is_self === 1) {
                    ...
                    // 获取账户列表
                    if ((is_self > 0 && group_id) || select_group_id) {
                        self.getList('#' + filters.account, layui.apiPath.userAccountList, {group_id: group_id}, 'username', '', select_user_id);
                    }
                }
                if (filters.shop) {
                    if (!select_shop_id) {
                    ...
                    // 获取店铺列表
                    if ((is_self > 0 && user_id) || select_user_id) {
                        self.getList('#' + filters.shop, layui.apiPath.userShopList, {user_id: user_id}, 'shop_name', '', select_shop_id);
                    }
                }
            ...
            //oss上传（直链）
            ,
            ossUpload: function (file, callback) {
                admin.req({
                    url : layui.apiPath.ossParams.url,
                    type: layui.apiPath.ossParams.type,
                    data: {
                        name: file.name
                    },
                    success(ret) {
                        ...
                        for (let i in ret.data) {
                            formData.append(i, ret.data[i]);
                        }
                        formData.append('OSSAccessKeyId', ret.data.id);
                        formData.append('file', file);
                        $.ajax({
                            url        : layui.apiPath.ossUrl,
                            type       : 'POST',
                            contentType: false,
                            processData: false,
                            data       : formData,
                            success(res) {
                                formData.append('url', ret.data.key);
                                formData.append('size', file.size);
                                formData.append('name', file.name);
                                formData.append('type', file.type);
                                $.ajax({
                                    url        : layui.apiPath.ossNotify.url,
                                    type       : layui.apiPath.ossNotify.type,
                                    ...
                                    success(rek) {
                                        ...
                                        callback({
                                            code  : 0,
                                            url   : layui.apiPath.ossUrl + ret.data.key,
                                            notify: rek.data
                                        });
                                    }
                                });
